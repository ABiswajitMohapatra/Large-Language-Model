...
def chat_with_agent_stream(query, index, chat_history, memory_limit=6, extra_file_content="",
                            uploaded_docs=None, last_uploaded=None, model=None, persona_prompt=None,
                            memory_context=None):
...
- Every slide needs "title" and "notes". Content slides need 3-5 concise "bullets" (max ~12 words each).
...
def _parse_slide_json(raw: str) -> dict:
    ...
    # Trim to the outermost JSON object in case the model added stray text.
...
def _add_content_slide(prs, theme, s):
    ...
    body = slide.shapes.add_textbox(Inches(0.8), Inches(1.6), Inches(11.5), Inches(5.3))
...
def execute_plugin(plugin_name: str, query: str):
    """
    ...
    """
    try:
        # Plugins are stored in browser localStorage, not backend.
        # So this is a stateless call - the frontend must pass the full
        # plugin definition OR we just handle generic parameters here.
        # For now: treat URL as a template with {query} placeholder.
        
        # Example plugin from frontend:
        # {name: "weather", url: "https://api.weather.com/forecast?q={query}", method: "GET"}
        
        # Since plugins are frontend-stored, we can't access them here directly.
        # Instead, we just validate + sanitize + return error.
        # The frontend should send full plugin details OR handle this client-side.
        
        if not plugin_name or not query:
            return {"success": False, "result": "", "error": "Plugin name and query required"}
        
        # Placeholder: in production, receive full plugin URL from frontend
        # and execute the API call here, then return result.
        return {
            "success": False,
            "result": "",
            "error": "Plugin system requires frontend to pass URL/method in request body. Update PluginExecuteRequest model."
        }
    except Exception as e:
